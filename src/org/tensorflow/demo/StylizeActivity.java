...
public class StylizeActivity extends CameraActivity implements OnImageAvailableListener {
	...
	// Whether to actively manipulate non-selected sliders so that sum of activations always
	// appears
	...
	private void setStyle(final ImageSlider slider, final float value) {
		...
		if (NORMALIZE_SLIDERS) {
			// Slider vals correspond directly to the input tensor vals, and normalization is
			// visually
	...
	public void onImageAvailable(final ImageReader reader) {
		...
		try {
			...
			if (desiredSize != initializedSize) {
				...
				rgbFrameBitmap = Bitmap.createBitmap(previewWidth, previewHeight, Config
						.ARGB_8888);
	...
	private void stylizeImage(final Bitmap bitmap) {
		...
		if (DEBUG_MODEL) {
			...
			for (int i = 0; i < intValues.length; ++i) {
				...
				final float val =
						Math.abs(x - centerX) < squareSize && Math.abs(
								y - centerY) < squareSize ? 1.0f : 0.0f;
				floatValues[i * 3] = val;
...

		{
					...
					new OnClickListener() {
						...
						public void onClick(final View v) {
							if (textureCopyBitmap != null) {
								...
								ImageUtils.saveBitmap(textureCopyBitmap,
										"stylized" + frameNum + ".png");
								Toast.makeText(
										StylizeActivity.this,
										"Saved image to: /sdcard/tensorflow/" + "stylized" +
												frameNum + ".png",
										Toast.LENGTH_LONG)
										.show();
							}
			...
			for (int i = 0; i < NUM_STYLES; ++i) {
				...
				if (items[i] == null) {
					...
					final Bitmap bm =
							getBitmapFromAsset(StylizeActivity.this,
